    ...
    <form action="{{ route('dashboard.payment.settings.store') }}" method="POST">
        ...
        <div>
            ...
            <div class="d-flex flex-wrap gap-3">
                <div class="mb-3">
                    <label for="queue_no" class="form-label">Sıra No</label>
                    <input type="number" class="form-control" name="queue_no" value="{{ $setting->queue_no }}">
                </div>
                <div class="mb-3">
                    <label for="payment_name" class="form-label">Ödeme Adı</label>
                    <input type="text" class="form-control" name="payment_name" value="{{ $setting->payment_name }}">
                </div>
                <div class="mb-3">
                    <label for="minimum_amount" class="form-label">Minimum Tutar</label>
                    <input type="text" class="form-control" name="minimum_amount" value="{{ $setting->minimum_amount }}">
                </div>
                <div class="mb-3">
                    <label for="maximum_amount" class="form-label">Maksimum Tutar</label>
                    <input type="text" class="form-control" name="maximum_amount" value="{{ $setting->maximum_amount }}">
                </div>
                <div class="mb-3">
                    <label for="status" class="form-label">
                        Durum
                ...
                <div class="mb-3">
                    <label for="pricing" class="form-label">
                        ...
                        @endphp
                        <select name="pricing">
                            @foreach($pricing as $value => $option)
                                @if($settings->status == $value)
                                    <option value="{{ $value }}" selected>{{ $option }}</option>
                                @endif
                                <option value="{{ $value }}">{{ $option }}</option>
                ...
                <div class="mb-3">
                    <label for="purchase_price" class="form-label">Ödeme Ücreti</label>
                    <input type="text" class="form-control" name="purchase_price" value="{{ $setting->purchase_price_2_limit }}">
                </div>
                <div class="mb-3">
                    <label for="purchase_price_2" class="form-label">Ödeme Ücreti 2</label>
                    <input type="text" class="form-control" name="purchase_price_2" value="{{ $setting->purchase_price_2_limit }}">
                </div>
                <div class="mb-3">
                    <label for="purchase_price_2_limit" class="form-label">Ödeme Ücreti 2 Sınır</label>
                    <input type="text" class="form-control" name="purchase_price_2_limit" value="{{ $setting->purchase_price_2_limit }}">
                </div>
            </div>
        </div>
        <button type="submit" class="btn btn-primary">Ödeme ayarını kaydet</button>
